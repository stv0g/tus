	...
	if ($_POST) {
		...
		echo '<form action="' . $site['path']['web'] . '/index.php?module=sis&amp;command=edit&amp;cat_id=' . $site['cat']['id'] . '" method="post">
				<table>
					...
					<tr><td>{lang:sis:liga_id} für ' . $site['cat']['name'] . '</td><td><input type="text" id="sis_liga" name="sis_liga" value="' . $line['sis_liga'] . '" /> <input type="button" onclick="popup(\'' . $site['path']['web'] . '/modules/sis/select_liga.php\', \'Liga auswählen\', 560, 380);" value="{lang:sis:select_liga}" /></td></tr>
	...
	switch ($_GET['type']) {
		case 'ranking':
			...
			if (!empty($xml)) {
				...
				if (!empty($site['usr']['id'])) echo '<div class="error">{lang:sis:no_id}</div>';
				trigger_error('No SiS data available', E_USER_NOTICE);
			}
			break;
		case 'matches':
		default:
			$sis = new sis($site['cat']['id'], 1);
			$xml = $sis->get_xml();	
